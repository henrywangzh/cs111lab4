...
// values below found online at http://www.nongnu.org/ext2-doc/ext2.html as suggested in comment below
...
void write_superblock(int fd) {
	...
	superblock.s_r_blocks_count    = 0; // not reserving anything
	...
	superblock.s_log_block_size    = 0; /* 1024 */ // 2^(0 + 10) = 1024
	superblock.s_log_frag_size     = 0; /* 1024 */
	superblock.s_blocks_per_group  = BLOCK_SIZE*8; /* 8192, a group uses 1 block, so it has 8192 bits worth of data, 0 = block is unused, 1 = block is used */
...
void write_block_bitmap(int fd) {
	...
	for(int i = 0; i < BLOCK_SIZE; i++) {
		//add padding to block bitmap from bit 1024 to 8192
		// since we aren't using those
		if(i > 127) {
			block_bitmap[i] = 0xFF;
		}
	}
	// we are using first 23 blocks
	block_bitmap[0] = 0xFF;
	block_bitmap[1] = 0xFF;
	block_bitmap[2] = 0x7F;
	// more padding
...
void write_inode_bitmap(int fd) {
	...
	for (int i = 0; i < BLOCK_SIZE; i++) {
	// total of 128 inodes, 128/8 = 16
	// first let's clear out those 128 inodes since we want to use those
		if (i < 16) {
			inode_bitmap[i] = 0x00;
		}
		else {
			// and pad the rest with 0xFF
			inode_bitmap[i] = 0xFF;
		}
	}

	// now that we have those cleared, and since we know the first 13 inodes are being used
	// we want to the first unused inode to be inode 14
	inode_bitmap[0] = 0xFF;
	inode_bitmap[1] = 0x1F;
...
struct ext2_inode root_inode = {0};
	...
	hello_world.i_mode = EXT2_S_IFREG | EXT2_S_IRUSR | EXT2_S_IWUSR | EXT2_S_IRGRP | EXT2_S_IROTH;
	...
	hello.i_mode = EXT2_S_IFLNK | EXT2_S_IRUSR | EXT2_S_IWUSR | EXT2_S_IRGRP | EXT2_S_IROTH;
	hello.i_uid = 1000;
	hello.i_size = 11;
